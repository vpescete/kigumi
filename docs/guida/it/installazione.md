...
Il binario operativo si chiama `meshble` ed è prodotto dal crate `meshble-cli`, che dichiara esplicitamente il nome del bin nel proprio `Cargo.toml`:
...
Tutti i comandi seguenti usano questo binario. Negli esempi è abbreviato in `meshble`; in un ambiente reale invocalo con il percorso completo `target/release/meshble`, oppure copialo in una directory nel `PATH`. Durante lo sviluppo puoi anche usare `cargo run -p meshble-cli -- <comando>`.
...
Non serve eseguire DDL a mano: `meshble migrate` (e `meshble serve`) creano e versionano tutti gli schemi. È sufficiente che il database esista e che l'utente in `DATABASE_URL` possa crearvi tabelle.
...
I segreti **non** vanno mai nel file `meshble.toml`: si forniscono esclusivamente dall'ambiente (o da un secret manager) e la loro presenza è verificata all'avvio. L'istanza fa fail-fast se manca un segreto obbligatorio.

| Variabile | Obbligatorietà | Scopo |
|---|---|---|
| `DATABASE_URL` | **Obbligatoria** | DSN PostgreSQL completo: identità di connessione. |
| `MESHBLE_JWT_SECRET` | **Obbligatoria** | Segreto di firma HS256 per i token di accesso/refresh. |
| `MESHBLE_ADMIN_PASSWORD` | Per il bootstrap | Password dell'utente `admin` creato al primo `serve` (vedi sotto). |
| `MESHBLE_SMTP_PASSWORD` | Condizionale | Obbligatoria **solo** se `[mail].smtp_host` è configurato; altrimenti il caricamento dei `Settings` fallisce. |
| `MESHBLE_JWT_SECRET_OLD` | Opzionale | Segreto JWT precedente, **riservato** alla rotazione: in v1 viene caricato in configurazione ma non ancora passato al verificatore (l'`Authenticator` riceve il solo `MESHBLE_JWT_SECRET`). |
| `MESHBLE_ADMIN_TOKEN` | Opzionale | Segreto **riservato** alla futura protezione delle operazioni distruttive sul db (dump/restore/gc): in v1 viene caricato ma l'enforcement non è ancora cablato (gli endpoint non esistono ancora). |
| `MESHBLE_NEW_PASSWORD` | Opzionale | Password per `meshble user create` / `set-password` quando non si passa `--password`. |

`DATABASE_URL` e `MESHBLE_JWT_SECRET` sono le due variabili **richieste** in assoluto: la lettura dell'ambiente fallisce se una di esse manca o è vuota. La verifica incrociata SMTP è esplicita: se in configurazione è presente `mail.smtp_host` ma manca `MESHBLE_SMTP_PASSWORD`, il caricamento dei `Settings` ritorna l'errore `"mail.smtp_host is set but MESHBLE_SMTP_PASSWORD is not"`.
...
meshble config print    # stampa la config redatta + i runtime setting dal db
```
...
2. la variabile d'ambiente `MESHBLE_CONFIG`;
3. il default `./meshble.toml` nella directory corrente.
...
```
defaults < meshble.toml < variabili d'ambiente MESHBLE_CONF_*
```

Le variabili `MESHBLE_CONF_*` mappano sulle sezioni del file usando il **doppio underscore** `__` come separatore di nidificazione. Il prefisso `MESHBLE_CONF_` è volutamente distinto dalle variabili dei segreti (`DATABASE_URL`, `MESHBLE_JWT_SECRET`, …), così i segreti non vengono mai catturati dal layer di configurazione.
...
[auth]
access_ttl = 900               # 15 min (segreto JWT via env MESHBLE_JWT_SECRET)
refresh_ttl = 2592000          # 30 giorni

[mail]
smtp_host = "smtp.acme.com"    # password SMTP via env MESHBLE_SMTP_PASSWORD
smtp_port = 587
from = "erp@acme.com"
```
...
```bash
meshble module list               # elenca i moduli linkati con versione, stato e summary
meshble module install account    # installa account + dipendenze, poi migra
meshble module uninstall sales    # smette di migrare/servire il modulo; tabelle e dati restano
```
...
2. **fa il bootstrap dell'admin** da `MESHBLE_ADMIN_PASSWORD` (vedi sotto);
...
createdb meshble
meshble migrate                 # schemi framework + base + sua chiusura
meshble module install sales    # moduli applicativi desiderati (+ dipendenze)
meshble serve                   # ri-migra, bootstrap admin, serve
```

## Bootstrap dell'admin

Al primo `serve`, se non esiste già un utente `admin`, il binario lo crea a partire da `MESHBLE_ADMIN_PASSWORD` (la password non è mai hardcoded). Se `MESHBLE_ADMIN_PASSWORD` non è impostata, il bootstrap viene saltato con l'avviso `"warning: no admin user; set MESHBLE_ADMIN_PASSWORD to bootstrap one"` e nessun admin viene creato.
...
La password può anche arrivare da `MESHBLE_NEW_PASSWORD` invece che da `--password`.

## Frontend web

Il frontend di amministrazione è una SPA React/Vite opzionale in `web/`. In sviluppo è un processo separato dal server Rust e instrada le chiamate API verso un'istanza `meshble serve` viva.
...
Il dev server di Vite ascolta sulla porta **5180** e fa da proxy verso il `meshble serve` in esecuzione (default `127.0.0.1:8099`), così il browser resta same-origin e non serve CORS. I path inoltrati sono:
...
Quindi in sviluppo avvia in parallelo il backend (`meshble serve`) e il frontend (`npm run dev`).
...
Gli asset statici prodotti in `web/dist/` vanno serviti da un web server / reverse proxy, instradando `/api`, `/auth` e `/openapi.json` verso il backend Meshble. Il server Rust è headless e **non** serve asset statici.

> Nota: la SPA in `web/` gira di default su dati mock in-memory (mockup navigabili del design-system) e non richiede il backend per essere navigata; il proxy verso `meshble serve` serve quando la si collega all'API reale. Altri comandi disponibili: `npm run preview` (anteprima della build) e `npm run typecheck` (`tsc --noEmit`).
...
- **`s3`**: la validazione della configurazione richiede `storage.bucket` (e si possono indicare `region`, con le credenziali via ambiente). Nota però che a livello di storage, in v1, è disponibile solo il backend su filesystem: la crate `meshble-storage` documenta `S3BlobStore` come fuori da v1. Vedi le **Incertezze**.

### Rotazione del segreto JWT

Il design prevede `MESHBLE_JWT_SECRET_OLD` per ruotare il segreto di firma JWT senza invalidare i token già emessi: si imposta `MESHBLE_JWT_SECRET` al nuovo valore e `MESHBLE_JWT_SECRET_OLD` al precedente, da mantenere accettato in verifica durante la finestra di rotazione. **Stato in v1**: il segmento è caricato in configurazione ma non ancora cablato a runtime — l'`Authenticator` riceve un solo segreto (`MESHBLE_JWT_SECRET`), quindi la verifica con il segreto precedente non è ancora attiva. Vedi anche [sicurezza.md](sicurezza.md).

### Operazioni distruttive

`MESHBLE_ADMIN_TOKEN` è un segreto **riservato** alla futura protezione delle operazioni distruttive sul database (dump/restore/gc): in v1 viene caricato nella configurazione ma l'enforcement non è ancora cablato e quei comandi/endpoint non esistono ancora. Coerente con [configurazione.md](configurazione.md).
