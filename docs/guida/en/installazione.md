...
The operational binary is named `meshble` and is produced by the `meshble-cli` crate, which explicitly declares the bin name in its own `Cargo.toml`:
...
```
target/release/meshble
```

All the commands below use this binary. In the examples it is abbreviated as `meshble`; in a real environment invoke it with the full path `target/release/meshble`, or copy it to a directory on the `PATH`. During development you can also use `cargo run -p meshble-cli -- <command>`.
...
`meshble` connects to an **already existing** database — it does not create one. Create it separately:

```bash
createdb meshble
```

There is no need to run DDL by hand: `meshble migrate` (and `meshble serve`) create and version all the schemas. It is enough for the database to exist and for the user in `DATABASE_URL` to be able to create tables in it.
...
Secrets must **never** go into the `meshble.toml` file: they are supplied exclusively from the environment (or from a secret manager), and their presence is verified at startup. The instance fails fast if a required secret is missing.

| Variable | Required | Purpose |
|---|---|---|
| `DATABASE_URL` | **Required** | Complete PostgreSQL DSN: connection identity. |
| `MESHBLE_JWT_SECRET` | **Required** | HS256 signing secret for access/refresh tokens. |
| `MESHBLE_ADMIN_PASSWORD` | For bootstrap | Password of the `admin` user created on the first `serve` (see below). |
| `MESHBLE_SMTP_PASSWORD` | Conditional | Required **only** if `[mail].smtp_host` is configured; otherwise loading the `Settings` fails. |
| `MESHBLE_JWT_SECRET_OLD` | Optional | Previous JWT secret, **reserved** for rotation: in v1 it is loaded into the configuration but not yet passed to the verifier (the `Authenticator` receives only `MESHBLE_JWT_SECRET`). |
| `MESHBLE_ADMIN_TOKEN` | Optional | Secret **reserved** for the future protection of destructive database operations (dump/restore/gc): in v1 it is loaded but the enforcement is not yet wired up (the endpoints do not exist yet). |
| `MESHBLE_NEW_PASSWORD` | Optional | Password for `meshble user create` / `set-password` when `--password` is not provided. |

`DATABASE_URL` and `MESHBLE_JWT_SECRET` are the two absolutely **required** variables: reading the environment fails if either of them is missing or empty. The SMTP cross-check is explicit: if `mail.smtp_host` is present in the configuration but `MESHBLE_SMTP_PASSWORD` is missing, loading the `Settings` returns the error `"mail.smtp_host is set but MESHBLE_SMTP_PASSWORD is not"`.
...
meshble config print    # prints the redacted config + the runtime settings from the db
```
...
2. the environment variable `MESHBLE_CONFIG`;
3. the default `./meshble.toml` in the current directory.

```bash
meshble --config /etc/meshble/meshble.toml serve
# or
export MESHBLE_CONFIG=/etc/meshble/meshble.toml
meshble serve
```
...
```
defaults < meshble.toml < MESHBLE_CONF_* environment variables
```

The `MESHBLE_CONF_*` variables map onto the file's sections using the **double underscore** `__` as the nesting separator. The `MESHBLE_CONF_` prefix is deliberately kept distinct from the secret variables (`DATABASE_URL`, `MESHBLE_JWT_SECRET`, …), so that secrets are never captured by the configuration layer.
...
```bash
meshble module list               # lists the linked modules with version, status, and summary
meshble module install account    # installs account + dependencies, then migrates
meshble module uninstall sales    # stops migrating/serving the module; tables and data remain
```
...
2. **bootstraps the admin** from `MESHBLE_ADMIN_PASSWORD` (see below);
...
createdb meshble
meshble migrate                 # framework schemas + base + its closure
meshble module install sales    # desired application modules (+ dependencies)
meshble serve                   # re-migrate, bootstrap admin, serve
```

## Admin bootstrap

On the first `serve`, if an `admin` user does not already exist, the binary creates one from `MESHBLE_ADMIN_PASSWORD` (the password is never hardcoded). If `MESHBLE_ADMIN_PASSWORD` is not set, the bootstrap is skipped with the warning `"warning: no admin user; set MESHBLE_ADMIN_PASSWORD to bootstrap one"` and no admin is created.
...
```bash
meshble user create alice --password 's3cret' --groups user,sales
meshble user set-password alice --password 'nuova'
meshble user grant alice admin
meshble user company alice --active 1 --allowed 1,2
```

The password can also come from `MESHBLE_NEW_PASSWORD` instead of `--password`.

## Web frontend

The administration frontend is an optional React/Vite SPA in `web/`. In development it is a process separate from the Rust server and routes API calls to a live `meshble serve` instance.
...
The Vite dev server listens on port **5180** and proxies to the running `meshble serve` (default `127.0.0.1:8099`), so the browser stays same-origin and CORS is not needed. The forwarded paths are:
...
So in development, start the backend (`meshble serve`) and the frontend (`npm run dev`) in parallel.
...
The static assets produced in `web/dist/` are to be served by a web server / reverse proxy, routing `/api`, `/auth`, and `/openapi.json` to the Meshble backend. The Rust server is headless and does **not** serve static assets.

> Note: the SPA in `web/` runs by default on in-memory mock data (navigable design-system mockups) and does not require the backend to be browsed; the proxy to `meshble serve` is needed when connecting it to the real API. Other available commands: `npm run preview` (preview of the build) and `npm run typecheck` (`tsc --noEmit`).
...
- **`s3`**: configuration validation requires `storage.bucket` (and you can specify `region`, with the credentials via the environment). Note, however, that at the storage level, in v1, only the filesystem backend is available: the `meshble-storage` crate documents `S3BlobStore` as out of v1. See the **Uncertainties**.

### JWT secret rotation

The design provides `MESHBLE_JWT_SECRET_OLD` to rotate the JWT signing secret without invalidating already-issued tokens: you set `MESHBLE_JWT_SECRET` to the new value and `MESHBLE_JWT_SECRET_OLD` to the previous one, to be kept accepted in verification during the rotation window. **Status in v1**: the segment is loaded into the configuration but not yet wired up at runtime — the `Authenticator` receives a single secret (`MESHBLE_JWT_SECRET`), so verification with the previous secret is not yet active. See also [sicurezza.md](sicurezza.md).

### Destructive operations

`MESHBLE_ADMIN_TOKEN` is a secret **reserved** for the future protection of destructive database operations (dump/restore/gc): in v1 it is loaded into the configuration but the enforcement is not yet wired up, and those commands/endpoints do not exist yet. Consistent with [configurazione.md](configurazione.md).
