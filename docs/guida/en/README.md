...
|---|---|---|
| Postgres schema (DDL + versioned migrations) | `kigumi-schema` / `kigumi-db` | tables, columns, FKs, constraints |
| Headless REST API | `kigumi-server` | secure CRUD and actions from the catalog |
| OpenAPI 3.1 specification | `kigumi-schema` | `GET /openapi.json` |
| Agnostic UI contract (JSON) | `kigumi-schema` | a generic frontend draws forms and tables *from the contract* |
| Security policy (ACLs + record rules) | `kigumi-core` | access enforced by the code path, not just by the data |
...
- **Fail-fast on secrets**: the instance refuses to start if a required secret is missing (`DATABASE_URL`, `KIGUMI_JWT_SECRET`).
...
| Crate | Role |
|---|---|
| `kigumi-core` | introspectable metamodel, AST domains, security (ACLs + record rules + sudo), compile-time registry, versioning |
| `kigumi-macros` | `#[model]` / `#[extend]` proc-macros |
| `kigumi-schema` | projections: Postgres DDL, JSON UI contract, OpenAPI 3.1 |
| `kigumi-db` | Postgres persistence (sqlx): security-enforced CRUD + versioned migrations |
| `kigumi-auth` | JWT HS256 auth (Bearer → trusted `Ctx`), password hashing |
| `kigumi-config` | typed, validated boot-time configuration + reading secrets from the environment |
| `kigumi-storage` | content-addressed blob store behind a trait (`BlobStore`) for attachments |
| `kigumi-server` | headless axum server: metadata + CRUD from the catalog |
...
and receives everything it needs — `#[model]`, `#[extend]`, `Ctx`, `Domain`, `Model`, `ModelDescriptor`, `ModuleManifest`, `ModuleDep`, the security types (`Acl`, `RecordRule`), and the registrars exposed as macros by the `kigumi` crate (`register_module!`, `register_acls!`, `register_rules!`, `register_action!`, `register_service!`, `register_route!`, `register_job!`, `register_sequence!`, `register_seed!`, `register_migration!`, and the like).

### Modules (`modules/`)

Each module declares a `MANIFEST` (`ModuleManifest`) with a name, a version, a framework compatibility range (the `framework` field), and dependencies (`depends`), and auto-registers with `kigumi::register_module!(MANIFEST)`.
...
| App | Binary | Role |
|---|---|---|
| `kigumi-cli` | `kigumi` | the single CLI to operate an instance: `serve`, `migrate`, `config`, `user`, `acl`, `rule`, `module`, `version` |
| `renderer-demo` | `kigumi-renderer-demo` | runnable demo: migrates + seeds a model and serves the API + a reference renderer |

The `kigumi` CLI links the module crates (`kigumi-mod-base`, `kigumi-mod-mail`, `kigumi-mod-sales`, `kigumi-mod-account`, `kigumi-mod-stock`); precisely because they are linked, their `inventory` registrations are present in the binary and the modules turn out to be *available* for installation. Only **installed** modules, however, are migrated and served.

### Web (`web/`)

`web/` is a Vite/React SPA: the reference web app for the admin UI. In development it runs as a separate process on port `5180` and proxies the `/api`, `/auth`, and `/openapi.json` paths to `kigumi serve` (default `127.0.0.1:8099`), so the browser stays same-origin (no CORS). The Rust server is headless and does **not** serve static assets: the web app is a separate client of the UI contract and the REST API.
...
The produced binary is `target/release/kigumi`. In the examples below you can use `cargo run -p kigumi-cli -- <command>` during development, or `kigumi <command>` if the binary is on the `PATH`.

### 2. Create the database

`kigumi` connects to an **already existing** database (it does not create it): create it separately.
...
Secrets are read **only** from the environment, never from the configuration file; the instance fails fast if a required one is missing. Required are `DATABASE_URL` (a complete Postgres DSN: the sole source of connection identity) and `KIGUMI_JWT_SECRET` (the HS256 secret for signing tokens).
...
### 4. Prepare `kigumi.toml`

Non-secret boot-time settings live in `kigumi.toml` (default: `./kigumi.toml`, or `$KIGUMI_CONFIG`, or `--config <path>`). The effective configuration is given by the layering `defaults < kigumi.toml < environment variables with the KIGUMI_CONF_ prefix` (nested with `__`, for example `KIGUMI_CONF_SERVER__BIND=0.0.0.0:9000`). The file is optional for commands that do not start the blob store, but `serve` requires `storage.path` when the storage backend is `fs` (the default): without it, validation fails fast. Start from the example:

```bash
cp kigumi.toml.example kigumi.toml
```

A minimal `kigumi.toml` for the quickstart:
...
On a fresh instance, the `admin` user is created from the `KIGUMI_ADMIN_PASSWORD` variable (no password is hardcoded). The bootstrap happens inside `serve`: if an `admin` user does not already exist and the variable is set, it is created with all the groups declared by the ACLs/record rules of the linked modules plus the base `user`/`admin` groups, and assigned to every existing company.

```bash
export KIGUMI_ADMIN_PASSWORD="$(openssl rand -base64 24)"
```

> If `KIGUMI_ADMIN_PASSWORD` is not set, `serve` still starts but prints a warning and no admin is created.
...
With `kigumi serve` running on `127.0.0.1:8099`, the `/api`, `/auth`, and `/openapi.json` paths are proxied to the backend, so the web app's calls reach the real API from the same origin.
...
| [configurazione.md](configurazione.md) | Configuration: `kigumi.toml`, secrets via the environment, `KIGUMI_CONF_*` overrides, runtime settings in the database. |
