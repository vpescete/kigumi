...
async fn validate_moves_stock_and_is_single_shot() {
    ...
    assert!(db.validate_picking(&su, &[], &[], receipt).await.is_err(), "a done transfer cannot be validated again");
    ...
    let n2 = db.validate_picking(&su, &[], &[], delivery).await.unwrap();
    ...
    assert!(db.validate_picking(&su, &[], &[], 999_999).await.is_err(), "unknown transfer id errors");
    ...
    assert!(db.validate_picking(&su, &[], &[], empty).await.is_err(), "an empty transfer cannot be validated");
