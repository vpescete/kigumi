//! Realized FX LOSS routes to an EXPENSE account (gains go to income; this proves the loss path). A USD
//! invoice booked at 100 EUR is paid when USD has strengthened so the cash is worth only 50 EUR; the
//! 50 EUR difference is a realized loss booked as an expense debit, and the entry still balances.
...
#[tokio::test]
async fn payment_books_a_realized_fx_loss() {
    link();
    ...
    // USD STRENGTHENS from 1.0 USD/EUR (invoice era) to 2.0 USD/EUR (payment era): 100 USD is worth 100
    // EUR when invoiced, only 50 EUR when paid — a 50 EUR realized LOSS.
    ins(&rate, json!({ "currency_id": usd, "name": "2020-01-01", "rate": "1.0", "company_id": comp })).await;
    ins(&rate, json!({ "currency_id": usd, "name": "2026-01-01", "rate": "2.0", "company_id": comp })).await;
    ...
    let exp = ins(&account, json!({ "code": "600000", "name": "Expenses", "account_type": "expense", "company_id": comp })).await;
    ...
    // A USD invoice dated 2020-06-01, booked at that era's rate (100 USD = 100 EUR), residual 100 USD.
    let move_id = db.insert_secured(&mv, &su, &[], &[], json!({
        ...
        "line_ids": [
            { "account_id": inc, "name": "Goods", "debit": "0", "credit": "100", "amount_currency": "-100", "partner_id": cust, "company_id": comp },
    ...
    assert!(pl.iter().any(|l| l["account_id"].as_i64() == Some(bank_acc) && money(l, "debit") == 50.0), "bank debit = 50 EUR at the payment rate");
    assert!(pl.iter().any(|l| l["account_id"].as_i64() == Some(recv) && money(l, "credit") == 100.0), "receivable relieved at the 100 EUR it was booked");
    assert!(pl.iter().any(|l| l["account_id"].as_i64() == Some(exp) && money(l, "debit") == 50.0), "50 EUR realized FX loss to EXPENSE (not income)");
