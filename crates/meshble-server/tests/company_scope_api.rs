...
use meshble_core::{resolve, Acl, FieldDef, FieldKind, ModelDescriptor, RecordRule, ResolvedModel};
use meshble_db::Db;
...
async fn login_scopes_data_to_the_users_company() {
    ...
    sqlx::query("DELETE FROM meshble_user WHERE login IN ('scoped', 'roamer')").execute(setup.pool()).await.unwrap();
    ...
    // Scope survives a refresh (re-read from meshble_user, like groups).
