...
use kigumi_core::{actions_for, delegated_fields, field_is_readonly, field_required_groups, is_mailed, json_string, related_path, reports_for, view_for, Domain, DomainError, FieldDef, FieldKind, ResolvedModel};
...
pub fn pg_add_column_type(field: &FieldDef) -> Option<String> {
    ...
    match field.kind {
        ...
        FieldKind::Image => col.push_str(" REFERENCES kigumi_attachment(id)"),
        _ => {}
    }
    Some(col)
}
...
pub fn to_ddl(m: &ResolvedModel) -> String {
    ...
    for f in m.fields.iter().filter(|f| f.has_column()) {
        ...
        match f.kind {
            FieldKind::Many2one { target } => col.push_str(&format!(" REFERENCES {}(id)", table_of(target))),
            // An Image references the attachment table directly (ir.attachment → kigumi_attachment).
            FieldKind::Image => col.push_str(" REFERENCES kigumi_attachment(id)"),
            _ => {}
        }
        if f.required {
...
mod tests {
    use super::*;
    use kigumi_core::{resolve, FieldDef, ModelDescriptor, ResolvedModel};
