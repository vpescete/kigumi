...
use meshble_core::{Acl, Domain, Operation, RecordRule, RuleDomain};
use sqlx::Row;
...
const ENSURE_RULE: &str = "CREATE TABLE IF NOT EXISTS meshble_rule \
...
impl Db {
    ...
    pub async fn set_db_rule(
        ...
        let id: i64 = sqlx::query_scalar(
            "INSERT INTO meshble_rule (model, grp, ops, domain) VALUES ($1, $2, $3, $4) RETURNING id",
        )
        .bind(model)
        .bind(groups)
        .bind(ops)
    ...
    pub async fn remove_db_rule(&self, id: i64) -> Result<(), DbError> {
        sqlx::query("DELETE FROM meshble_rule WHERE id = $1").bind(id).execute(&self.pool).await?;
        Ok(())
    }

    /// Lists the configured DB record rules (for `meshble rule list`).
    pub async fn list_db_rules(&self) -> Result<Vec<RuleRow>, DbError> {
        let rows = sqlx::query(
            "SELECT id, model, grp, ops, domain, active FROM meshble_rule ORDER BY model, id",
        )
        .fetch_all(&self.pool)
        .await?;
        Ok(rows
