...
//! restart and the DATABASE is their single authority (boot-time config lives in meshble.toml/env;
//! see docs/OPERATIONS.md §2.1). Keys like `base_url`, `mode`, `neutralized`, `banner` live here.

use crate::{Db, DbError};
use sqlx::Row;

const ENSURE_SETTING: &str = "CREATE TABLE IF NOT EXISTS meshble_setting \
...
impl Db {
    ...
    pub async fn set_setting(&self, key: &str, value: &str, vtype: &str) -> Result<(), DbError> {
        sqlx::query(
            "INSERT INTO meshble_setting (key, value, vtype) VALUES ($1, $2, $3) \
    ...
    pub async fn get_setting(&self, key: &str) -> Result<Option<String>, DbError> {
        let row = sqlx::query("SELECT value FROM meshble_setting WHERE key = $1")
            .bind(key)
    ...
    pub async fn all_settings(&self) -> Result<Vec<(String, String, String)>, DbError> {
        let rows = sqlx::query("SELECT key, value, vtype FROM meshble_setting ORDER BY key")
            .fetch_all(&self.pool)
            .await?;
        Ok(rows
            .iter()
...
mod tests {
    ...
    async fn settings_seed_set_and_read() {
        ...
        sqlx::query("DELETE FROM meshble_setting WHERE key = 'test.k'").execute(db.pool()).await.unwrap();
