...
const ENSURE_SEQUENCE: &str = "CREATE TABLE IF NOT EXISTS meshble_sequence \
...
impl Db {
    ...
    pub async fn ensure_sequence(
        ...
        sqlx::query(
            "INSERT INTO meshble_sequence (code, prefix, suffix, padding) VALUES ($1, $2, $3, $4) \
    ...
    pub async fn next_value(&self, code: &str) -> Result<String, DbError> {
        let row = sqlx::query(
            "UPDATE meshble_sequence SET next_number = next_number + step \
...
mod tests {
    ...
    async fn sequence_formats_and_advances() {
        ...
        sqlx::query("DELETE FROM meshble_sequence WHERE code = 'TST'")
            .execute(db.pool())
            .await
            .unwrap();
