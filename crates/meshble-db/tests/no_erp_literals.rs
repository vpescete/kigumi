//! Framework-purity guard: meshble-db is the framework crate and must name NO ERP business model. If this
...
fn core_names_no_erp_model() {
    ...
    assert!(
        hits.is_empty(),
        "ERP business-model literals leaked into meshble-db/src — relocate them to the owning module \
