...
use meshble_core::{resolve, Acl, Ctx, FieldDef, FieldKind, ModelDescriptor, ModelRegistration, Value};
use meshble_db::Db;
use serde_json::json;

// `qty_doubled` reads the record's own `qty`; `label` builds a string from it. Both on-read.
fn orc_double(i: &meshble_core::ComputeInput) -> Value {
    Value::Int(i.int("qty") * 2)
}
fn orc_label(i: &meshble_core::ComputeInput) -> Value {
    Value::Str(format!("qty={}", i.int("qty")))
}
meshble_core::inventory::submit! { meshble_core::ComputeRegistration { name: "orc_double", func: orc_double } }
meshble_core::inventory::submit! { meshble_core::ComputeRegistration { name: "orc_label", func: orc_label } }
...
meshble_core::inventory::submit! { ModelRegistration { name: "orc.item", module: "test", descriptor: item } }
