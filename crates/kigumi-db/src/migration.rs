...
//! declarative, versioned SQL with a SemVer target. State is tracked in `kigumi_module` (current
//! version) and `kigumi_migration` (one row per applied version, for audit + per-version
...
use kigumi_core::ResolvedModel;
use kigumi_schema::{pg_add_column_type, to_ddl};
use semver::Version;
use sqlx::Row;
...
const ENSURE_MODULE: &str =
    "CREATE TABLE IF NOT EXISTS kigumi_module (name text PRIMARY KEY, version text NOT NULL)";
const ENSURE_HISTORY: &str = "CREATE TABLE IF NOT EXISTS kigumi_migration \
     (module text NOT NULL, version text NOT NULL, applied_at timestamptz NOT NULL DEFAULT now(), \
      PRIMARY KEY (module, version))";
const UPSERT_MODULE: &str = "INSERT INTO kigumi_module (name, version) VALUES ($1, $2) \
     ON CONFLICT (name) DO UPDATE SET version = EXCLUDED.version";

impl Db {
    /// Whether this database was migrated BEFORE module-selection existed — i.e. the per-model
    /// migration ledger `kigumi_module` has rows. Used by `migrate` to tell a truly-fresh DB (install
    ...
    pub async fn has_prior_migration(&self) -> Result<bool, DbError> {
        let reg: Option<String> =
            sqlx::query_scalar("SELECT to_regclass('kigumi_module')::text").fetch_one(&self.pool).await?;
        if reg.is_none() {
            return Ok(false);
        }
        let count: i64 = sqlx::query_scalar("SELECT count(*) FROM kigumi_module").fetch_one(&self.pool).await?;
        Ok(count > 0)
    }
    ...
    pub async fn install_or_upgrade(
        ...
        let installed: Option<String> = sqlx::query("SELECT version FROM kigumi_module WHERE name = $1")
            .bind(module)
            ...
            Some(installed_str) => {
                ...
                if installed_v >= target {
                    ...
                    let applied: HashSet<String> =
                        sqlx::query("SELECT version FROM kigumi_migration WHERE module = $1")
                            .bind(module)
                            .fetch_all(&mut *tx)
                            .await?
                            .iter()
                    ...
                    for (v, m) in &pending {
                        ...
                        sqlx::query("INSERT INTO kigumi_migration (module, version) VALUES ($1, $2)")
                            .bind(module)
                            .bind(v.to_string())
                            .execute(&mut *tx)
                            .await?;
