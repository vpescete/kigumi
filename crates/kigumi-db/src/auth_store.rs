...
const ENSURE_USER: &str = "CREATE TABLE IF NOT EXISTS kigumi_user \
...
const ENSURE_USER_COMPANY: &str =
    "ALTER TABLE kigumi_user ADD COLUMN IF NOT EXISTS company_id bigint, \
     ADD COLUMN IF NOT EXISTS company_ids text NOT NULL DEFAULT ''";
const ENSURE_REFRESH: &str = "CREATE TABLE IF NOT EXISTS kigumi_refresh \
...
impl Db {
    ...
    pub async fn upsert_user(
        ...
        let id: i64 = sqlx::query_scalar(
            "INSERT INTO kigumi_user (login, password_hash, groups) VALUES ($1, $2, $3) \
    ...
    pub async fn user_groups(&self, uid: i64) -> Result<Vec<String>, DbError> {
        let row = sqlx::query("SELECT groups FROM kigumi_user WHERE id = $1")
            .bind(uid)
            .fetch_optional(&self.pool)
            .await?;
    ...
    pub async fn user_scope(&self, uid: i64) -> Result<(Option<i64>, Vec<i64>), DbError> {
        let row = sqlx::query("SELECT company_id, company_ids FROM kigumi_user WHERE id = $1")
            .bind(uid)
            .fetch_optional(&self.pool)
            .await?;
        Ok(row
    ...
    pub async fn set_user_companies(
        ...
        let n = sqlx::query("UPDATE kigumi_user SET company_id = $1, company_ids = $2 WHERE login = $3")
            .bind(active)
            .bind(csv)
            .bind(login)
            .execute(&self.pool)
    ...
    pub async fn store_refresh(&self, jti: &str, uid: i64, ttl_secs: i64) -> Result<(), DbError> {
        sqlx::query(
            "INSERT INTO kigumi_refresh (jti, user_id, expires_at) \
    ...
    pub async fn refresh_user(&self, jti: &str) -> Result<Option<i64>, DbError> {
        let row = sqlx::query(
            "SELECT user_id FROM kigumi_refresh WHERE jti = $1 AND NOT revoked AND expires_at > now()",
        )
        .bind(jti)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn revoke_refresh(&self, jti: &str) -> Result<(), DbError> {
        sqlx::query("UPDATE kigumi_refresh SET revoked = true WHERE jti = $1")
            .bind(jti)
            .execute(&self.pool)
            .await?;
        Ok(())
