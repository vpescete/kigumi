...
kigumi_core::inventory::submit! { ModelRegistration { name: "sec.tpl", module: "test", descriptor: tpl } }
kigumi_core::inventory::submit! { ModelRegistration { name: "sec.var", module: "test", descriptor: var } }
kigumi_core::inventory::submit! { InheritsRegistration { model: "sec.var", parent: "sec.tpl", via: "tpl_id" } }
// The parent's `cost` is manager-only (D6). It must stay restricted when read/written THROUGH the variant.
kigumi_core::inventory::submit! { FieldGroupRegistration { model: "sec.tpl", field: "cost", groups: &["mgr"] } }
